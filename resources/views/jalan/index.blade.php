...
@section('title', 'Daftar Laporan Jalan')
@section('page-title', 'Daftar Laporan Jalan')
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="laporanTable">
                    ...
                    <tbody>
                        @foreach($laporanJalan as $index => $laporan)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $laporan->pelapor }}</td>
                            <td>
                                {{ ucfirst($laporan->jenis_rusak) }}
                            </td>
                            <td>
                                {{ $laporan->lokasi }}
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Laporan Jalan Baru</h5>
            ...
            <div class="modal-body">
                <form id="addLaporanForm">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="jenis_rusak" required>
                            <option value="jalan rusak ringan">Jalan Rusak Ringan</option>
                            <option value="jalan rusak berat">Jalan Rusak Berat</option>
                        </select>
                    </div>
                    <div class="form-group">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <img id="fotoPreview" src="" alt="Foto Kerusakan Jalan" class="img-fluid">
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Apakah Anda yakin ingin menghapus laporan jalan di lokasi <span id="lokasiText"></span>?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
...
<div class="modal fade" id="rekapModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Rekap Laporan Jalan</h5>
                <button type="button" class="close" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>
...
$(document).ready(function() {
    ...
    table = $('#laporanTable').DataTable({
        ...
        columnDefs: [
            ...
            { responsivePriority: 2, targets: 2 }, // Jenis Kerusakan
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        row.remove().draw();
                        alert('Laporan jalan berhasil dihapus');
                    } else {
                        alert('Gagal menghapus laporan jalan');
                    }
                },
                error: function() {
                    alert('Terjadi kesalahan saat menghapus laporan jalan');
                }
            });
        }
    ...
    // Tambahkan fungsi untuk menampilkan peta lokasi
    ...
    $('#exportRekap').click(function() {
        ...
        let url = `/jalan/rekap/${format}?tahun=${tahun}`;
        if (periode === 'bulan') {
            url += `&bulan=${bulan}`;
        }
...
$('#saveLaporan').on('click', function(e) {
    ...
    $.ajax({
        url: '{{ route("jalan.store") }}', // Ganti dengan route yang sesuai
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
